/**
 * Chart Module: Health Facility Province Distribution
 * 
 * This module handles the creation of the health facility province distribution chart and table.
 */

/**
 * Renders the entire health facility province distribution section including chart and table.
 * @param {string} containerId - The ID of the container element.
 * @param {Array} data - The dataset from the AEFI dashboard.
 */
function renderHealthFacilityProvinceDistribution(containerId, data) {
    ...
    header.innerHTML = `
        <h3 class="chart-title">Health Facility State or Province</h3>
    ...
    chartContent.innerHTML = '<canvas id="healthFacilityProvincesChart"></canvas>';
    ...
    // Render chart
    renderHealthFacilityProvinceChart(sortedProvinces);

    // Render table
    renderHealthFacilityProvinceTable(tableContent, sortedProvinces);
...
/**
 * Renders the Health Facility Province Distribution bar chart.
 * @param {Array} sortedProvinces - Sorted array of [province, count] pairs.
 */
function renderHealthFacilityProvinceChart(sortedProvinces) {
    const config = {
        type: 'bar',
        data: {
            labels: sortedProvinces.map(p => p[0]),
            datasets: [{
                label: 'Cases by Health Facility Province',
    ...
    createChartEnhanced('healthFacilityProvincesChart', config.type, config.data, config.options);
}

/**
 * Renders the data table for health facility province distribution.
 * @param {HTMLElement} tableContainer - The container to render the table in.
 * @param {Array} sortedProvinces - Sorted array of [province, count] pairs.
 */
function renderHealthFacilityProvinceTable(tableContainer, sortedProvinces) {
